...
class LSTMCNNConcatRelationClassifier(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim,
                 n_ent_labels, n_pos_labels, n_rel_labels, n_loc_labels,
                 pretrained_emb=None, freeze_emb=False):
        super(LSTMCNNConcatRelationClassifier, self).__init__()
        self.vocab_size = vocab_size
        self.embed_dim = embed_dim
        self.hidden_dim = hidden_dim
        self.n_rel_labels = n_rel_labels
        ...
        self.dropout = nn.Dropout(0.5)
        self.fc_cnn = nn.Linear(hidden_dim, n_rel_labels)
        ...
        self.fc_lstm = nn.Linear(2*hidden_dim, hidden_dim)
        # The linear layer that maps from hidden state space to tag space
        self.feat2tag = nn.Linear(4*hidden_dim, n_rel_labels)
    ...
    def forward(self, words, locs1, locs2, ents, poss, ps, cs, p_lengths, c_lengths, word_lengths):
        ...
        # CNN.
        wemb, lemb1, lemb2, pemb, eemb = w_emb, locs1_emb, locs2_emb, pos_emb, ent_emb
        ...
        f_cnn = torch.cat(max_pool_outs, dim=1)
        # scores_cnn = self.fc_cnn(self.dropout(f_cnn))
        ...
        sent_hn = sent_hn.transpose(0, 1).contiguous().view(batch_size, -1)
        sent_hn = sent_hn[unsorted_word_idx]
        sent_hn = self.fc_lstm(sent_hn)

        # Concatenate features.
        # features = p_hn + c_hn + sent_hn + f_cnn
        
        features = torch.cat((p_hn, c_hn, sent_hn, f_cnn), dim=1)
        scores = self.feat2tag(features)
